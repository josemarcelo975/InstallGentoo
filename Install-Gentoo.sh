...
dialog \
 ...
 --msgbox "In label type select dos and create a ROOT and SWAP partition.mark ROOT as bootable and SWAP select 82 Linux swap / Solaris." \
  15 55
  cfdisk
  partitionroot=$(dialog --menu "What is Your ROOT partition?" 0 0 0 \
  /dev/sda1 "" \
  /dev/sda2 "" --stdout)
  yes | mkfs.ext4 $partitionroot
  partitionswap=$(dialog --menu "What is Your SWAP partition?" 0 0 0 \
  /dev/sda1 "" \
  /dev/sda2 "" --stdout)
